...
import { 
  ...
  Clock, Lock, Sun, Moon, Globe, Layers
} from 'lucide-react';
import { 
...
const OmsTradingEnginePanel = React.lazy(() => import('./components/OmsTradingEnginePanel').then(m => ({ default: m.OmsTradingEnginePanel })));
const SentimentIntelligencePanel = React.lazy(() => import('./components/SentimentIntelligencePanel').then(m => ({ default: m.SentimentIntelligencePanel })));
...
export default function App() {
  ...
  // Right sidebar panel adjustable width state (default 420px for comfortable reading)
  const [rightPanelWidth, setRightPanelWidth] = useState<number>(420);

  const handleMouseDownResizeRightPanel = (e: React.MouseEvent) => {
    e.preventDefault();
    ...
    const handleMouseMove = (moveEvent: MouseEvent) => {
      ...
      const newWidth = Math.min(Math.max(startWidth + deltaX, 300), 960);
      ...
      <main className="flex-1 flex overflow-hidden">
        ...
        {!maximizedPaneId && (
          <div className={`flex h-full border-l ${isLight ? 'border-slate-200' : 'border-[#2e3242]'}`}>
            ...
            {activeRightPanel && (
              <div 
                ...
                className={`relative ${isLight ? 'bg-white border-r border-slate-200' : 'bg-[#171b26] border-r border-[#2e3242]'} flex flex-col h-full overflow-hidden transition-[width] duration-75`}
              >
                {/* Drag Handle on Left Border */}
                <div
                  onMouseDown={handleMouseDownResizeRightPanel}
                  className="absolute left-0 top-0 bottom-0 w-2 cursor-col-resize hover:bg-violet-500/60 active:bg-violet-500 z-30 transition-colors flex items-center justify-center group"
                  title="Drag to resize panel width"
                >
                  <div className="w-0.5 h-8 bg-gray-600/40 group-hover:bg-violet-300 rounded" />
                </div>

                <div className={`p-3 border-b ${isLight ? 'border-slate-200 bg-white' : 'border-[#2e3242] bg-[#171b26]'} flex items-center justify-between sticky top-0 z-10 shrink-0 gap-2`}>
                  <div className="flex items-center gap-2 overflow-hidden">
                    <h2 className={`text-xs font-bold uppercase tracking-wider truncate ${isLight ? 'text-slate-800' : 'text-gray-200'}`}>
                      ...
                      {activeRightPanel === 'oms' && 'OMS & Algo Engine'}
                      {activeRightPanel === 'sentiment' && 'Sentiment & Macro Hub'}
                    </h2>
                  </div>

                  <div className="flex items-center gap-1.5 shrink-0">
                    {/* Width Preset Buttons */}
                    <div className="flex items-center bg-[#0d1017] p-0.5 rounded border border-gray-800 text-[9px] font-mono">
                      {[320, 480, 640, 800].map((w) => (
                        <button
                          key={w}
                          onClick={() => setRightPanelWidth(w)}
                          className={`px-1.5 py-0.5 rounded cursor-pointer ${
                            Math.abs(rightPanelWidth - w) < 30
                              ? 'bg-violet-600 text-white font-bold'
                              : 'text-gray-400 hover:text-gray-200'
                          }`}
                          title={`Set panel width to ${w}px`}
                        >
                          {w === 320 ? 'S' : w === 480 ? 'M' : w === 640 ? 'L' : 'XL'}
                        </button>
                      ))}
                    </div>
                    ...
                    <button 
                      ...
                      className={`cursor-pointer p-1 rounded hover:bg-gray-800/50 ${isLight ? 'text-slate-400 hover:text-slate-700' : 'text-gray-400 hover:text-white'}`}
                      title="Close Panel"
                    >
                      <X className="w-4 h-4" />
                    </button>
              ...
              <div className={`p-0 flex-1 flex flex-col ${['terminal', 'backtest', 'autotrade', 'pinescript', 'agents', 'vault', 'oms', 'sentiment'].includes(activeRightPanel) ? 'overflow-hidden' : 'overflow-y-auto scrollbar-thin'}`}>
                ...
                {activeRightPanel === 'oms' && (
                  <React.Suspense fallback={<div className="p-6 text-center text-xs font-mono text-gray-500">Loading Order Management Engine...</div>}>
                    <OmsTradingEnginePanel
                      pane={focusedPane}
                      candles={focusedData}
                      balance={prefs.accountBalance}
                      positions={positions}
                      closedTrades={closedTrades}
                      onExecuteTrade={(trade) => handleExecuteTrade(trade.direction, trade.quantity, trade.slDistance || 0, trade.tpDistance || 0)}
                      isLight={isLight}
                    />
                  </React.Suspense>
                )}
                ...
                {activeRightPanel === 'sentiment' && (
                  <React.Suspense fallback={<div className="p-6 text-center text-xs font-mono text-gray-500">Loading Sentiment & Intelligence Hub...</div>}>
          ...
          <div className="w-[72px] shrink-0 bg-[#121620] flex flex-col items-center py-4 gap-2.5 z-20 border-l border-gray-800/60 overflow-y-auto scrollbar-none">
            ...
            {!isSimpleMode && (
            <button
              onClick={() => setActiveRightPanel(prev => prev === 'oms' ? null : 'oms')}
              className={`w-full py-2 px-0.5 flex flex-col items-center justify-center gap-1 cursor-pointer transition-colors ${activeRightPanel === 'oms' ? 'bg-blue-600/15 text-blue-400 border-r-2 border-blue-500' : 'text-gray-400 hover:bg-[#1e2235] hover:text-blue-200'}`}
              title="Order Management System & Algorithmic Trading"
            >
              <Layers className="w-4.5 h-4.5" />
              <span className="text-[8px] font-sans font-semibold tracking-tighter uppercase text-center scale-90">OMS Algo</span>
            </button>
            )}

            {/* 10. Sentiment & Intelligence Hub */}
            {!isSimpleMode && (
            <button
              onClick={() => setActiveRightPanel(prev => prev === 'sentiment' ? null : 'sentiment')}
              className={`w-full py-2 px-0.5 flex flex-col items-center justify-center gap-1 cursor-pointer transition-colors ${activeRightPanel === 'sentiment' ? 'bg-violet-600/15 text-violet-400 border-r-2 border-violet-500' : 'text-gray-400 hover:bg-[#1e2235] hover:text-violet-200'}`}
              title="Social Media & Geopolitical Intelligence"
            >
              <Globe className="w-4.5 h-4.5" />
              <span className="text-[8px] font-sans font-semibold tracking-tighter uppercase text-center scale-90">Sentiment</span>
            </button>
            )}

            {/* 11. Key Vault (Pro only) */}
            {!isSimpleMode && (
            <button
