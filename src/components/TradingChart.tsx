...
export const TradingChart: React.FC<TradingChartProps> = ({
  ...
  const [hoveredTime, setHoveredTime] = useState<any>(null);
  const hoveredTimeRef = useRef<any>(null);

  const rsiSeriesRef = useRef<any>(null);
  const macdSeriesRef = useRef<any>(null);
  const cvdSeriesRef = useRef<any>(null);
  const obvMacdSeriesRef = useRef<any>(null);
  const pineSeriesRef = useRef<any>(null);

  const rsiDataRef = useRef<{ time: any; value: number }[]>([]);
  const macdDataRef = useRef<{
    macd: { time: any; value: number }[];
    signal: { time: any; value: number }[];
    hist: { time: any; value: number; color: string }[];
  }>({ macd: [], signal: [], hist: [] });
  const cvdDataRef = useRef<{ time: any; value: number }[]>([]);
  const obvMacdDataRef = useRef<any[]>([]);
  const pineDataRef = useRef<any[]>([]);

  const isSyncingLocalCrosshairRef = useRef<boolean>(false);

  // Local crosshair synchronization across main chart + all subcharts
  const syncLocalCrosshair = useCallback((sourceChart: IChartApi | null, param: any) => {
    if (isSyncingLocalCrosshairRef.current) return;
    isSyncingLocalCrosshairRef.current = true;

    try {
      const time = param?.time;
      const point = param?.point;

      // Update hovered time state
      const newTime = (param && param.time !== undefined) ? param.time : null;
      if (newTime !== hoveredTimeRef.current) {
        hoveredTimeRef.current = newTime;
        setHoveredTime(newTime);
      }

      const chartsToSync = [
        { chart: chartRef.current, series: candleSeriesRef.current, isMain: true },
        { chart: rsiChartRef.current, series: rsiSeriesRef.current },
        { chart: macdChartRef.current, series: macdSeriesRef.current },
        { chart: cvdChartRef.current, series: cvdSeriesRef.current },
        { chart: obvMacdChartRef.current, series: obvMacdSeriesRef.current },
        { chart: pineChartRef.current, series: pineSeriesRef.current }
      ];

      for (const item of chartsToSync) {
        if (!item.chart || item.chart === sourceChart) continue;

        if (time !== undefined && point !== undefined) {
          let value = 0;
          if (item.isMain) {
            const candle = historicDataRef.current?.find(c => c && c.time === time);
            value = candle ? candle.close : 0;
          } else if (item.series) {
            // Find appropriate value at time
            if (item.chart === rsiChartRef.current) {
              const pt = rsiDataRef.current?.find(d => d && d.time === time);
              value = pt ? pt.value : 50;
            } else if (item.chart === macdChartRef.current) {
              const pt = macdDataRef.current?.hist?.find(d => d && d.time === time);
              value = pt ? pt.value : 0;
            } else if (item.chart === cvdChartRef.current) {
              const pt = cvdDataRef.current?.find(d => d && d.time === time);
              value = pt ? pt.value : 0;
            } else if (item.chart === obvMacdChartRef.current) {
              const pt = obvMacdDataRef.current?.find(d => d && d.time === time);
              value = pt ? pt.obvMacd : 0;
            } else if (item.chart === pineChartRef.current) {
              const firstPlot = pineDataRef.current?.find(p => !p.overlay);
              const pt = firstPlot?.data?.find((d: any) => d && d.time === time);
              value = pt ? pt.value : 0;
            }
          }
          try {
            if (item.series) {
              item.chart.setCrosshairPosition(value, time, item.series);
            }
          } catch (err) {
            try { item.chart.clearCrosshairPosition(); } catch {}
          }
        } else {
          try { item.chart.clearCrosshairPosition(); } catch {}
        }
      ...
      console.error("Error in syncLocalCrosshair", err);
    } finally {
      isSyncingLocalCrosshairRef.current = false;
    }
  }, []);

  const getValueAtTime = useCallback((dataArray: any[], time: any, key: string = 'value') => {
    if (!dataArray || dataArray.length === 0) return null;
    const targetTime = time !== null ? time : dataArray[dataArray.length - 1]?.time;
    if (!targetTime) return null;
    const found = dataArray.find(d => d && d.time === targetTime);
    if (!found) return null;
    return found[key] !== undefined ? found[key] : found;
  }, []);
  ...
  useEffect(() => {
    ...
    const onCrosshairMove = (param: any) => {
      ...
      // Sync sub-charts inside this pane locally
      syncLocalCrosshair(chart, param);
    };
    ...
    const onSyncCrosshair = (e: any) => {
      ...
      // Also sync sub-charts inside this pane locally
      syncLocalCrosshair(chart, param);
  ...
  useEffect(() => {
    ...
    const mHistSeries = chart.addSeries(HistogramSeries, { priceLineVisible: false, lastValueVisible: false });
    mHistSeries.setData(hist as any);
    macdSeriesRef.current = mHistSeries;
  ...
  useEffect(() => {
    ...
    const obvMacdSeries = chart.addSeries(LineSeries, { color: '#d946ef', lineWidth: 2.5, priceLineVisible: false, lastValueVisible: false, title: '' });
    obvMacdSeries.setData(obvMacdData);
    obvMacdSeriesRef.current = obvMacdSeries;
    obvMacdDataRef.current = results;
    ...
    return () => {
      ...
      obvMacdSeriesRef.current = null;
      obvMacdDataRef.current = [];
    };
  ...
  useEffect(() => {
    ...
    if (pineStrategyOutput?.plots) {
      pineDataRef.current = pineStrategyOutput.plots;
      let firstSeriesSet = false;
      pineStrategyOutput.plots
    ...
    >
      ...
      {showRSI && (
        <div className="flex-shrink-0 flex flex-col border-t border-[#1e222e]" style={{ height: `${rsiHeight}px` }}>
          ...
          <div className="text-[8px] font-mono text-gray-450 px-2 flex-shrink-0 bg-[#090b10] flex justify-between items-center h-5 border-b border-[#1e222e]/40">
            <div className="flex items-center gap-2">
              <span className="text-gray-500 font-semibold">RSI ({pane.indicators.rsiLength || 14}):</span>
              {(() => {
                const val = getValueAtTime(rsiDataRef.current, hoveredTime);
                return val !== null ? (
                  <span className="text-[#f59e0b] font-bold">{val.toFixed(2)}</span>
                ) : (
      ...
      {showMACD && (
        <div className="flex-shrink-0 flex flex-col border-t border-[#1e222e]" style={{ height: `${macdHeight}px` }}>
          ...
          <div className="text-[8px] font-mono text-gray-450 px-2 flex-shrink-0 bg-[#090b10] flex justify-between items-center h-5 border-b border-[#1e222e]/40">
            <div className="flex items-center gap-2">
              <span className="text-gray-500 font-semibold">MACD ({pane.indicators.macdParams?.join('/') || '12/26/9'}):</span>
              {(() => {
                const macdVal = getValueAtTime(macdDataRef.current.macd, hoveredTime);
                const sigVal = getValueAtTime(macdDataRef.current.signal, hoveredTime);
                const histVal = getValueAtTime(macdDataRef.current.hist, hoveredTime);
                return (macdVal !== null && sigVal !== null && histVal !== null) ? (
                  <div className="flex items-center gap-3">
                    <span className="text-[#3b82f6]">MACD <span className="font-bold">{macdVal.toFixed(4)}</span></span>
                    <span className="text-[#f59e0b]">Signal <span className="font-bold">{sigVal.toFixed(4)}</span></span>
                    <span className={histVal >= 0 ? "text-emerald-400" : "text-rose-400"}>Hist <span className="font-bold">{histVal.toFixed(4)}</span></span>
                  </div>
                ) : (
                  <span className="text-gray-600">—</span>
                );
      ...
      {showCVD && (
        <div className="flex-shrink-0 flex flex-col border-t border-[#1e222e]" style={{ height: `${cvdHeight}px` }}>
          ...
          <div className="text-[8px] font-mono text-gray-450 px-2 flex-shrink-0 bg-[#090b10] flex justify-between items-center h-5 border-b border-[#1e222e]/40">
            <div className="flex items-center gap-2">
              <span className="text-gray-500 font-semibold">Cumulative Volume Delta (CVD):</span>
              {(() => {
                const val = getValueAtTime(cvdDataRef.current, hoveredTime);
                return val !== null ? (
      ...
      {showObvMacdDoubleMacd && (
        <div className="flex-shrink-0 flex flex-col border-t border-[#1e222e]" style={{ height: `${obvMacdHeight}px` }}>
          ...
          <div className="text-[8px] font-mono text-gray-450 px-2 flex-shrink-0 bg-[#090b10] flex justify-between items-center h-5 border-b border-[#1e222e]/40">
            <div className="flex items-center gap-2">
              <span className="text-gray-500 font-semibold">OBV MACD + Double MACD Combined:</span>
              {(() => {
                const targetTime = hoveredTime !== null ? hoveredTime : (obvMacdDataRef.current.length > 0 ? obvMacdDataRef.current[obvMacdDataRef.current.length - 1]?.time : null);
                if (!targetTime) return <span className="text-gray-600">—</span>;
                const found = obvMacdDataRef.current.find(d => d && d.time === targetTime);
                return found ? (
                  <div className="flex items-center gap-3 flex-wrap">
                    <span className="text-[#d946ef]">OBV <span className="font-bold">{found.obvMacd?.toFixed(2) || '—'}</span></span>
                    <span className="text-[#06b6d4]">L-MACD <span className="font-bold">{found.longMacd?.toFixed(4) || '—'}</span></span>
                    <span className="text-[#f43f5e]">L-Sig <span className="font-bold">{found.longSignal?.toFixed(4) || '—'}</span></span>
                    <span className="text-[#3b82f6]">S-MACD <span className="font-bold">{found.shortMacd?.toFixed(4) || '—'}</span></span>
                    <span className="text-[#eab308]">S-Sig <span className="font-bold">{found.shortSignal?.toFixed(4) || '—'}</span></span>
                  </div>
                ) : (
                  <span className="text-gray-600">—</span>
                );
      ...
      {hasPineOscillators && (
        <div className="flex-shrink-0 flex flex-col border-t border-[#1e222e]" style={{ height: `${pineHeight}px` }}>
          ...
          <div className="text-[8px] font-mono text-gray-450 px-2 flex-shrink-0 bg-[#090b10] flex justify-between items-center h-5 border-b border-[#1e222e]/40">
            <div className="flex items-center gap-2">
              <span className="text-gray-500 font-semibold">Pine Indicators ({pane.pineStrategy?.name || 'Indicators'}):</span>
              {(() => {
                const targetTime = hoveredTime !== null ? hoveredTime : (visibleData && visibleData.length > 0 ? visibleData[visibleData.length - 1]?.time : null);
                if (!targetTime) return <span className="text-gray-600">—</span>;
                const activePlots = pineDataRef.current?.filter(p => !p.overlay) || [];
                if (activePlots.length === 0) return null;
                return (
                  <div className="flex items-center gap-3 flex-wrap">
                    {activePlots.map((plot, pIdx) => {
                      const pt = plot.data?.find((d: any) => d && d.time === targetTime);
                      const val = pt ? pt.value : null;
                      return (
                        <span key={plot.id || pIdx} style={{ color: plot.color }}>
                          {plot.title} <span className="font-bold">{val !== null ? val.toFixed(4) : '—'}</span>
                        </span>
                      );
                    })}
                  </div>
