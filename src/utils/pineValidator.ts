    ...
    try {
        ...
        self.onmessage = function(e) {
          ...
          try {
            ...
            const fullJsCode = 
              ...
              ${JSON.stringify(PINE_HELPERS_BLOCK)} + "\\n" +
              "{\\n" +
              jsCode + "\\n" +
              "}";
            const runner = new Function('_candles_', '_params_', '_context_', fullJsCode);
            const output = runner(candles, params, { candles, params });
      ...
      try {
        const fullCode = 
          ...
          jsCode + "\n" +
          "}";
        const runner = new Function('_candles_', '_params_', '_context_', fullCode);
        const paramsMap: Record<string, any> = {};
        parameters.forEach((p) => {
          paramsMap[p.key] = p.value !== undefined ? p.value : p.default;
        });
        runner(last100Candles, paramsMap, { candles: last100Candles, params: paramsMap });
        resolve({ success: true });
      } catch (errInner: any) {
