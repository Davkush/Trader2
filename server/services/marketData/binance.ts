...
export async function fetchBinanceData(symbol: string, timeframe: string, limit = 600): Promise<CandleData[]> {
  ...
  const primaryUrl = `https://api.binance.com/api/v3/klines?symbol=${binanceSymbol}&interval=${binanceInterval}&limit=${fetchLimit}`;
  const fallbackUrl = `https://api.binance.us/api/v3/klines?symbol=${binanceSymbol}&interval=${binanceInterval}&limit=${fetchLimit}`;

  let response = await fetch(primaryUrl).catch(() => null);

  if (!response || !response.ok) {
    // Attempt Binance US endpoint if global Binance is geoblocked (HTTP 451 / 403)
    response = await fetch(fallbackUrl).catch(() => null);
  }

  if (!response || !response.ok) {
    const statusText = response ? `HTTP ${response.status}` : 'Network failure';
    throw new Error(`Binance endpoint unavailable (${statusText})`);
  }
